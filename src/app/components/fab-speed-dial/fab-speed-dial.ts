...
export class FabSpeedDialActionsComponent implements AfterContentInit {
    ...
    @ContentChildren(MatButton) _buttons: QueryList<MatButton> | any;
     ...
     */
    private showMiniFabAnimation: ReturnType<typeof setTimeout> | any;

    /**
     * When we will remove mini-fab buttons from DOM, after the animation is complete
     */
    private hideMiniFab: Subscription | any;

    constructor(injector: Injector, private renderer: Renderer2) {
        this._parent = injector.get(FabSpeedDialComponent);
    }
    ...
    private initButtonStates(): void {
        this._buttons.forEach((button: any, i: number) => {
    ...
    hide(): void {
        ...
        const obs = this._buttons.map((button: any, i: number) => {
            let opacity = '1';
            let transitionDelay = 0;
            let transform;
...
export class FabSpeedDialComponent implements OnDestroy, AfterContentInit {
    ...
    @ContentChild(FabSpeedDialActionsComponent) _childActions: FabSpeedDialActionsComponent | any;
